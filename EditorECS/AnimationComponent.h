...
struct AnimationClip
{
	unsigned int StartFrame;										///< 시작 프레임
	unsigned int EndFrame;											///< 끝 프레임
	float		 TickPerFrame;										///< 프레임 당 틱
	float		 FrameSpeed;										///< 프레임 속도
	bool		 LoopState = true;									///< 루프 상태


	std::map<std::string, std::vector<Matrix>>	LerpFrameMatrixList; ///< FBXLoader::PreProcess()의 GenerateAnimationTrack()에서 생성된 보간 행렬
	std::vector<Notifier*>						NotifierList;		 ///< 노티파이어 리스트
	std::wstring								FileName;			 ///< Fbx 파일의 이름
	std::string									FilePath;			 ///< Fbx 파일의 경로
};
...
public:
	...
	float									elapsedT = 0.f;								///< 애니메이션 블렌딩을 위한 경과 시간 정보
	float									lastFrame = 0.f;							///< 애니메이션 블렌딩을 위한 직전 애니메이션의 마지막 프레임 값
