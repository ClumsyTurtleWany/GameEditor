...
public: //외부 사용(래핑?)
	...
	static void Init()
	{
		for (int32 i = 0; i < UINT16_MAX; i++)
			GPacketHandler[i] = TestPacketHandler::Handle_INVALID;
		...
		{return TestPacketHandler::HandlePacket<protocol::C_TEST>(Handle_C_TEST, session, buffer, len); };
		GPacketHandler[PKT_C_MOVE] = [](PacketSessionRef& session, BYTE* buffer, int32 len)
		{return TestPacketHandler::HandlePacket<protocol::C_MOVE>(Handle_C_MOVE, session, buffer, len); };
	}
