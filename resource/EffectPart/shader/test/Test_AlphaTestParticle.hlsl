//////////////////////////////////////
//
// Test_AlphaTestParticle.hlsl
//		PNCT정점 구조를 사용하는 테스트용 알파테스팅 파티클 shader
...
void GS(in point VS_OUTPUT gIn[1], inout TriangleStream<PS_INPUT> gOut)
{
	...
	if (gIn[0].rot.z > 0.0f)
	{
		...
		float2 newT[4] = {
			{gIn[0].normal.x, gIn[0].normal.y},
			{gIn[0].uvCoord.x, gIn[0].normal.y},
			{gIn[0].normal.x, gIn[0].uvCoord.y},
			{gIn[0].uvCoord.x, gIn[0].uvCoord.y}
		};
